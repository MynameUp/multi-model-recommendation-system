...
class NewsRecommend:
    ...
    def writeToMySQL(self):
        ...
        for row in self.result:
            ...
            # 核心修改：使用 ON DUPLICATE KEY UPDATE 语法
            sql_i = """
                INSERT INTO news_api_recommend(userid, newsid, hadread, cor, species, time) 
                VALUES (%s, %s, 0, %s, 0, %s)
                ON DUPLICATE KEY UPDATE 
                cor = VALUES(cor), time = VALUES(time)
            """
            try:
                # 使用参数化传递数据，更加安全且不易报错
                self.cursor.execute(sql_i, (int(row[0]), int(row[1]), float(row[2]), time))
                self.db.commit()
            except Exception as e:
                # 打印出具体的错误 e，方便以后如果还有其他错误能直接看出来
                logger.error("写入/更新失败，错误信息: {}，rollback:{}".format(e, row))
                self.db.rollback()
