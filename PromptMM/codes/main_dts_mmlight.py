...
class RecSysDynamicTauScheduler:
    def __init__(self, initial_tau=3.0, min_tau=1.0, max_epoch=1000, momentum=0.9):# 3.0 1.0 1000 0.9, 3.0, 2.0, 50, 0.8
    ...
    def step(self, current_epoch, current_kd_loss):
        ...
        # progress = min(1.0, current_epoch / self.max_epoch)
        cosine_factor = 0.5 * (1 + math.cos(math.pi * progress))
        
        # 2. 自适应缩放因子
        adaptive_scale = current_kd_loss / (current_kd_loss + 0.1) 
        # adaptive_scale = current_kd_loss / (current_kd_loss + 0.05) 
...
#     def __init__(self, tau_max=3.0, tau_min=1.0, decay_epochs=30):
...
#         # 计算当前的线性温度
#         current_tau = self.tau_max - (self.tau_max - self.tau_min) * (epoch / self.decay_epochs)
#         return current_tau
...
class Trainer(object):
    ...
    def train(self):
        ...
        # 【新增】：初始化动态温度调度器
        # initial_tau 取你传入的参数，min_tau 设为 1.0 (最终降温目标)
        ...
        # 修改为使用新的线性调度器
        # dynamic_tau_scheduler = LinearTauScheduler(
        #     tau_max=args.student_tau,  # 初始温度，对应你命令里的 3.0
        #     tau_min=1.0,               # 最低温度
        #     decay_epochs=30            # 我们定好的 30 轮降到底
        # )

        last_epoch_kd_loss = 1.0 # 初始给个默认值
        # ============================================================== 
        ...
        for epoch in range(args.epoch):
            ...
            dynamic_tau_List.append(current_dynamic_tau) # <--- 【新增】：将当前温度加入列表
            
            total_kd_loss_this_epoch = 0.0 # 用于累计计算散度


            # === [修改后：获取本轮的线性动态温度] ===
            # 删除了 last_epoch_kd_loss，只传 epoch
            # current_dynamic_tau = dynamic_tau_scheduler.step(epoch)
            
            # self.logger.logging(f"Epoch {epoch} | 当前动态温度 (Dynamic Tau): {current_dynamic_tau:.4f}")
            # tb_writer.add_scalar('Student/Dynamic_Tau', current_dynamic_tau, epoch)

            # dynamic_tau_List.append(current_dynamic_tau) # 将当前温度加入列表
            
            # total_kd_loss_this_epoch = 0.0 # 用于累计计算散度 (这行保留不动)
            # ========================================
            ...
            for idx in tqdm(range(n_batch)):
                ...
                # student_batch_loss = 2.0*batch_mf_loss + batch_emb_loss + args.kd_loss_list_rate*kd_loss_list_image + args.kd_loss_list_rate*kd_loss_list_text + args.kd_loss_feat_rate*kd_loss_feat + kd_loss_list_dkd
                ...
                kd_loss_List.append(kd_loss_list.item()) # List-wise 蒸馏损失 (如果你想记的话)
