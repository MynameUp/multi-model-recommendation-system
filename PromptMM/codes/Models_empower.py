...
class PromptLearner(nn.Module):
    def __init__(self, image_feats=None, text_feats=None, ui_graph=None):
        ...
        # [原理分析] Hard Token 初始化策略
        # 为了让 Prompt 更好地适应多模态特征分布，作者没有使用随机高斯分布初始化，
        # 而是使用了降维算法 (PCA, ICA 等) 对原始图像/文本特征进行处理。
        # 这在图中可能表现为从 Data -> Pre-computed Features -> Hard Tokens 的流程。
        ...
        # 将降维后的特征均值作为 Item 的 Hard Token 初始值
        ...
        # 通过图结构传播，生成 User 的 Hard Token (User 表示为交互过的 Item 的聚合)
        self.user_hard_token = torch.mm(ui_graph, self.item_hard_token).cuda()
        
        # 定义线性变换层，将 Hard Token 转换为可学习的 Soft Prompt
    ...
    def forward(self):
        ...
        # [原理分析] 生成 Soft Prompt
        # 输出经过变换和 Dropout 的 Prompt 向量，这些向量随后会被“注入”到教师模型中
