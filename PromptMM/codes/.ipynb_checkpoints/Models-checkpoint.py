...
# 1. 新增：专门用于 Prompt 生成的 MoE 层
class MoE_Prompt_Layer(nn.Module):
    def __init__(self, embed_size, num_experts=4, top_k=2):
        super(MoE_Prompt_Layer, self).__init__()
        self.num_experts = num_experts
        self.top_k = top_k
        self.embed_size = embed_size

        # 定义 K 个专家，每个专家负责一个特定的子空间映射
        self.experts = nn.ModuleList([
            nn.Linear(embed_size, embed_size) for _ in range(num_experts)
        ])
        
        # 定义门控网络，负责根据输入 Token 决定求助哪些专家
    ...
    def forward(self, x):
        # x shape: [N, embed_size] (N 可以是 n_users 或 n_items)
        
        # 计算门控权重 (Gating logits)
        gate_logits = self.gate(x) 
        
        # 训练时注入正态分布噪声，鼓励探索未使用的专家
        # if self.training:
        #     noise = torch.randn_like(gate_logits) * 0.1 # 0.1 是噪声强度
        #     gate_logits = gate_logits + noise
            
        # 选出 Top-K 个最匹配的专家
        # indices: [N, top_k], weights: [N, top_k]
        weights, indices = torch.topk(gate_logits, self.top_k, dim=-1)
        
        # 在选出的专家之间进行 Softmax 归一化
        weights = F.softmax(weights, dim=-1) 
        
        # 初始化输出
        final_prompt = torch.zeros_like(x)
        
        # 获取所有专家的输出
        # expert_outputs shape: [N, num_experts, embed_size]
        expert_outputs = torch.stack([expert(x) for expert in self.experts], dim=1)
        
        # 遍历选中的 Top-K 专家，进行加权求和
        for i in range(self.top_k):
            # 获取当前选中的专家索引和权重
            expert_idx = indices[:, i]  # [N]
            expert_weight = weights[:, i].unsqueeze(1)  # [N, 1]
            
            # 提取对应专家的输出
            # [N, embed_size]
            selected_expert_output = expert_outputs[torch.arange(x.size(0), device=x.device), expert_idx]
            
            # 加权累加到最终的 prompt 中
            final_prompt += expert_weight * selected_expert_output
        # 在 forward 的 return 之前可以临时加入：
        if not self.training: # 只在测试或某几个step打印
            unique, counts = torch.unique(indices, return_counts=True)
            print(f"专家调用分布: {dict(zip(unique.tolist(), counts.tolist()))}")    
        return final_prompt
...
class PromptLearner(nn.Module):
    def __init__(self, image_feats=None, text_feats=None, ui_graph=None):
        ...
        # [画图原理] Hard Token 初始化
        # 论文图中可能展示了从 "Modal Features" 到 "Hard Tokens" 的箭头
        # 代码实现：使用 PCA/ICA 对原始特征降维，作为初始 Token
        if args.hard_token_type=='pca':
        #使用 PCA 将高维特征 (4096/1024) 降维到 embed_size (32)
            try:
                t1 = time()
        ...
        # 将降维后的特征均值(图像和文本 Hard Token 的均值)作为 Item 的初始 Prompt
        ...
        # User Hard Token: 通过图传播，聚合用户交互过的 Item 的 Token
        # 公式: H_user = A_ui * H_item
        self.user_hard_token = torch.mm(ui_graph, self.item_hard_token).cuda()

        # [画图原理] Soft Token 转换
        # --- 核心修改点：将单一线性层替换为 MoE 结构 ---直接使用 args 传入 MoE 参数 ---
        self.trans_user = MoE_Prompt_Layer(
            embed_size=args.embed_size, 
            num_experts=args.num_experts, 
            top_k=args.top_k
        ).cuda()
        
        self.trans_item = MoE_Prompt_Layer(
            embed_size=args.embed_size, 
            num_experts=args.num_experts, 
            top_k=args.top_k
        ).cuda()
        # ----------------------------------------------
        # 对应图中 Token 经过 Linear Layer 变成 Soft Prompt 的过程
    ...
    def forward(self):
        #基于图神经网络 (GNN) 的 Prompt 生成方式，是作者在实验过程中尝试过但最终未采用
