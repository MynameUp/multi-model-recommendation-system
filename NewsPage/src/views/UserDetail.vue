              ...
              <Col :xxs="1" :xs="5" :md="10" :lg="18" >
                <Card >
                  ...
                  <el-upload
                    ...
                    action="#"
                    :http-request="handleLocalAvatarUpload"
                    :show-file-list="false"
                    :before-upload="beforeAvatarUpload">
                    <img v-if="userAvatarDisplay" :src="userAvatarDisplay" class="avatar">
...
// 💡 修改点 2：引入上传 API 和头像解析工具
import { getUserdetail, updateUser, getTags, updateTags, uploadAvatarFile } from '@/api'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 💡 修改点 3：专门的视图展示变量，它会拼接 localhost:8000/media/...
      userAvatarDisplay: getAvatarPath(sessionStorage.getItem('userImg')),
  ...
  methods: {
    // 💡 修改点 4：替换原来的 handleAvatarSuccess，改用直连 Django 的上传逻辑
    async handleLocalAvatarUpload(options) {
      const { file } = options;
      this.$Loading.start();
      ...
      try {
        ...
        uploadFormData.append('userid', sessionStorage.getItem('userId'));

        const uploadRes = await uploadAvatarFile(uploadFormData);

        if (uploadRes.status === "100" || uploadRes.status === 100) {
          const fileName = uploadRes.data ? uploadRes.data.filename : null;

          if (fileName) {
            this.userdetail.headportrait = fileName;
            sessionStorage.setItem('userImg', fileName);
            this.userAvatarDisplay = getAvatarPath(fileName);

            this.$Message.success('修改头像成功！');
            setTimeout(() => {
              this.$router.go(0); // 刷新页面同步给顶部的 HeaderMenu
            }, 800);
          } else {
            this.$Message.error('头像上传成功但未返回文件名');
          }
        } else {
          this.$Message.error('头像上传失败: ' + (uploadRes.message || '未知错误'));
        }
      } catch (error) {
        console.error('头像上传错误:', error);
        this.$Message.error('网络请求异常或服务器错误');
      } finally {
        this.$Loading.finish();
      }
    },
    beforeAvatarUpload(file) {
      ...
      if (!this.displayornot) {
        this.$Message.warning('游客模式不可修改头像，请注册正式账号体验完整功能！');
        return false;
      }
    ...
    updateuserTags() {
      if (!this.displayornot) {
        this.$Message.warning('游客模式不可更新专属标签！');
        return;
      }
      let tags = []
      ...
      if (typeof param.seriesIndex != 'undefined') {
        ...
        let option = {
          ...
          series: [{
            ...
            gridSize: 3,
            textPadding: 2,
            autoSize: { enable: true, minSize: 10 },
            maskImage: maskImage,
            textStyle: {
    ...
    ok() {
      // 💡 新增：游客拦截机制
      if (!this.displayornot) {
        this.$Message.warning('游客模式不可修改账户信息！');
        return;
      }
    ...
    addTagok() {
      if (!this.displayornot) {
        this.$Message.warning('游客模式不可添加专属标签！');
        return;
      }
      if(this.newtag !== ''){
        let tags = []
  ...
  mounted() {
    ...
    getUserdetail(sessionStorage.getItem('userId')).then(res => {
      ...
      // 更新头像显示
      const avatarName = res.userdetail.headportrait || ''
      this.userdetail['headportrait'] = avatarName
      this.userAvatarDisplay = getAvatarPath(avatarName)
    })
